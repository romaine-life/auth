// Pure server-rendered HTML helpers for the Voight-Kampff landing,
// dashboard, and admin console. No DB, no Better Auth, no env access —
// everything is derived from the structural inputs passed in. This makes
// the views unit-testable / snapshot-renderable without standing up the
// full auth stack.
//
// Design tokens, components, and copy are distilled from the Claude
// Design handoff bundle "Voight-Kampff Auth" prototype; see the PR
// description on nelsong6/auth#17 for the source.
...
export const APP_INVENTORY: Array<{ host: string; name: string; key: string }> = [
...
/* Voight-Kampff theme — orange CRT on inky brown. */
body {
  --vk-accent: #ff7a3a;
...
.admin-grid textarea:focus { outline: none; border-color: var(--vk-accent); }
...
// ── Pretty-printed JSON for the Subject Profile decoded view ──────────────
...
// ── Page chrome: TopBar + Footer + SHELL ───────────────────────────────────

export type ShellOpts = { status?: "ok" | "pending"; signedIn?: boolean };

function renderTopBar(opts: ShellOpts) {
...
function renderFooter(opts: ShellOpts) {
...
export function shell(title: string, body: ReturnType<typeof html>, opts: ShellOpts = {}) {
...
// Loose structural shapes so the render module doesn't depend on the drizzle
// schema. Caller passes plain objects.
export type DashboardUser = {
  ...
  createdAt?: Date | null;
  role?: string;
  apps?: string; // JSON-serialized record
};
...
export function renderDashboard(opts: {
  ...
  const { user, accounts, sessions, currentSessionId } = opts;
  const role = user.role ?? "user";

  const appsBlob: Record<string, J> = (() => {
    try { return JSON.parse(user.apps ?? "{}") as Record<string, J>; }
    catch { return {} as Record<string, J>; }
  })();
  ...
  const inc = user.createdAt
    ? `${String(user.createdAt.getUTCDate()).padStart(2, "0")}·${MONTH_ABBR[user.createdAt.getUTCMonth()]}·${user.createdAt.getUTCFullYear()}`
    : "—";
  const subjectShort = user.id.length > 12 ? `${user.id.slice(0, 6)}…${user.id.slice(-4)}` : user.id;
  ...
  return html`
    <div class="dash">
      <div class="dash-head">
        ...
        <div class="head-text">
          <div class="head-name">${user.name}</div>
          <div class="head-email">${user.email} · ${user.id}</div>
...
export function renderAdmin(opts: { users: AdminUser[]; flash?: string }) {
  const { users, flash } = opts;
  return html`
    <div class="dash">
